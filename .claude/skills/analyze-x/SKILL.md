...
- **`{UPDATE_DIR}/scrape.json`** — object keyed by handle, each value an array of post objects with: `text`, `time`, `url`, `displayName`, `externalLinks`, `images`, `metrics`
- **`UPDATE_DIR`** — from pipeline orchestrator, used to locate scrape file and name analysis output
...
Save structured analysis to **`{UPDATE_DIR}/analysis.json`** with this schema:
